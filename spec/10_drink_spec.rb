...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      # beverage type.
      subject(:my_choice) { described_class.new('diet coke', 12) }
      # remove the 'x' before running this test
      it 'is your choice of beverage' do
        expect(my_choice.type).to eq('diet coke')
      end

      # remove the 'x' before running this test
      it 'has 16 ounces' do
        expect(my_choice.ounces).to eq(12)
      end
    end
  end
  ...
  describe '#full?' do
    context 'when drink has 16 ounces or more' do
      # Create an explicit subject, using 'described_class' and your choice of
      # beverage type.
      subject(:second_choice) { described_class.new('dr pepper', 16) }
      # remove the 'x' before running this test
      it 'is full' do
        expect(second_choice).to be_full
      end
    end
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:third_choice) { described_class.new('root beer', 13) }
      # remove the 'x' before running this test
      it 'is not full' do
        expect(third_choice).not_to be_full
      end
    end
  end
end
