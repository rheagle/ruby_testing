...
describe FindNumber do
  ...
  describe '#game_over?' do
    let(:game_is_over) { double('random_number', value: 7) }
    ...
    context 'when guess and random_number are equal' do
      ...
      subject(:guess_eql_rdm_num) { described_class.new(0, 9, game_is_over, 7) }
      ...
      it 'is game over' do
        expect(guess_eql_rdm_num).to be_game_over
      end
    end
    ...
    context 'when guess and random_number are not equal' do
      subject(:guess_not_eql_rdm_num) { described_class.new(0, 9, game_is_over, 4) }

      it 'is not game over' do
        expect(guess_not_eql_rdm_num).not_to be_game_over
      end
    end
  end
